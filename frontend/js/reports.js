(async () => {
  ...
  async function renderBody(){
    ...
    let data = [];
    try { data = (await Api.get('/reports/' + active)).data; } catch (err) { toast(err.message, 'error'); body.innerHTML = `<div class="panel no-tabs">${err.message}</div>`; return; }
    ...
    document.getElementById('report-body').innerHTML = `
      ...
      <div class="panel no-tabs">
        <div class="table-scroll"><table class="data-table">
          <thead><tr><th>Month</th><th>Project Code</th><th>Project Name</th><th>Billable Hours</th><th>Non-Billable Hours</th><th>Total Hours</th></tr></thead>
          <tbody>${data.map(r => `<tr><td>${r.month}</td><td>${r.project_code}</td><td>${r.project_name}</td><td>${hrs(r.billable_hours)}</td><td>${hrs(r.non_billable_hours)}</td><td><b>${hrs(r.total_hours)}</b></td></tr>`).join('') || emptyRow(6)}</tbody>
        </table></div>
      </div>`;
  }
  ...
  function renderEmployeewiseSummary(data){
    ...
    document.getElementById('report-body').innerHTML = `
      ...
      <div class="panel no-tabs">
        <div class="table-scroll"><table class="data-table">
          <thead><tr><th>Employee Code</th><th>Employee Name</th><th>Project Code</th><th>Project Name</th><th>Billable Hours</th><th>Non-Billable Hours</th><th>Total Hours</th></tr></thead>
          <tbody>${data.map(r => `<tr><td>${r.employee_code}</td><td>${r.full_name}</td><td>${r.project_code}</td><td>${r.project_name}</td><td>${hrs(r.billable_hours)}</td><td>${hrs(r.non_billable_hours)}</td><td><b>${hrs(r.total_hours)}</b></td></tr>`).join('') || emptyRow(7)}</tbody>
        </table></div>
      </div>`;
  }
  ...
  function renderProjectProfitability(data){
    ...
    document.getElementById('report-body').innerHTML = `
      ...
      <div class="panel no-tabs">
        <div class="table-scroll"><table class="data-table">
          <thead><tr><th>Project Code</th><th>Project Name</th><th>Billable Hours</th><th>Rate</th><th>Revenue</th><th>Cost</th><th>Profit</th><th>Margin %</th></tr></thead>
          <tbody>${data.map(r => `<tr><td>${r.project_code}</td><td>${r.project_name}</td><td>${hrs(r.billable_hours)}</td><td>${money(r.rate)}</td><td>${money(r.revenue)}</td><td>${money(r.cost)}</td><td style="color:${r.profit >= 0 ? '#16a34a' : '#dc2626'}; font-weight:700;">${money(r.profit)}</td><td>${r.marginPct}%</td></tr>`).join('') || emptyRow(8)}</tbody>
        </table></div>
      </div>`;
  }
  ...
  function emptyRow(cols){ return `<tr><td colspan="${cols}" style="text-align:center;color:#9ca3af;padding:28px;">No data available yet. Run Settings → Migration or add time entries first.</td></tr>`; }
